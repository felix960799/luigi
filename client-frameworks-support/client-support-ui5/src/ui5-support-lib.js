sap.ui.define(
  ['sap/ui/model/json/JSONModel', '@luigi-project/client/luigi-client'],
  function(JSONModel, LuigiClient) {
    ...
    return {
      ...
      connectTo: function(oComponent) {
        ...
        // Create preload view
        sap.ui.define(
          'luigi-ui5-support-lib/LuigiPreloadView',
          ['sap/ui/core/mvc/View', 'sap/ui/core/HTML'],
          function(View, HTML) {
            const PreloadView = View.extend(
              'luigi-ui5-support-lib.LuigiPreloadView',
              {
        ...
        oComponent.getRouter().attachRouteMatched(oEvent => {
          ...
          if (currentRouteObj && Object.keys(currentRouteObj).length > 0) {
            ...
            if (currentRouteObj.data && currentRouteObj.data.luigiRoute) {
              ...
              currentRouteObj.data &&
              currentRouteObj.data.fromVirtualTreeRoot
            ) {
              let url = location.hash.split('#/')[1];
              const truncate =
                currentRouteObj.data.fromVirtualTreeRoot.truncate;
              if (truncate) {
            ...
            if (ux.isModal()) {
              if (currentRouteObj.data.updateModalDataPath) {
                lm.updateModalPathInternalNavigation(
                  route,
                  {},
                  currentRouteObj.data.addHistoryEntry
                );
              }
            } else if (
              currentRouteObj.data?.preventBrowserHistory === true &&
              route
            ) {
              ...
              LuigiClient.linkManager()
                ...
                .then(sOldLuigiRoute => {
                  sOldLuigiRoute =
                    sOldLuigiRoute.charAt(0) === '/'
                      ? sOldLuigiRoute.substring(1)
                      : sOldLuigiRoute;
